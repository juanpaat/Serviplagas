...
def generate_report_in_memory(df_preventivo, df_roedores, df_lamparas):
    ...
    fig1 = generate_order_area_plot(df_preventivo)
    add_plot_to_doc(doc, fig1)

    doc.add_heading("Preventivos 2", level=2)
    fig2= generate_plagas_timeseries_facet(df_preventivo)
    add_plot_to_doc(doc, fig2)

    doc.add_heading("Preventivos 3", level=2)
    fig3 = generate_total_plagas_trend_plot(df_preventivo)
    add_plot_to_doc(doc, fig3)

    doc.add_heading("Roedores 1", level=2)
    fig1 = generate_roedores_station_status_plot(df_roedores)
    add_plot_to_doc(doc, fig1)

    doc.add_heading("Roedores 2", level=2)
    fig2= plot_tendencia_eliminacion_mensual(df_roedores)
    add_plot_to_doc(doc, fig2)

    doc.add_heading("Lámparas 1", level=2)
    fig2= plot_estado_lamparas_por_mes(df_lamparas)
    add_plot_to_doc(doc, fig2)

    doc.add_heading("Lámparas 2", level=2)
    fig2= plot_estado_lamparas_con_leyenda(df_lamparas)
    add_plot_to_doc(doc, fig2)

    doc.add_heading("Lámparas 3", level=2)
    fig2= plot_capturas_especies_por_mes(df_lamparas)
    add_plot_to_doc(doc, fig2)

    doc.add_heading("Lámparas 4", level=2)
    fig2= plot_tendencia_total_capturas(df_lamparas)
    add_plot_to_doc(doc, fig2)
